<template>
    ...
    <div class="col-4">
      <VueMultiselect
        ...
        <template v-if="filter.project_states.length" #beforeList class="multiselect__element">
          <span @click="handleDiselectProjects"
    ...
    <div class="col-4">
      <VueMultiselect
        ...
        <template #beforeList class="multiselect__element">
          <span @click="handleSelectProjects"
                class="multiselect__option diselect_all"><span>Select Planned Projects</span></span>
        </template>
      </VueMultiselect>
    </div>
  </div>
  ...
  <div class="row">
    <div class="col-4 d-flex justify-content-start">
      <button :disabled="!loaded" type="button" class="btn btn-primary date-change-button"
              @click="handleDateInput(false)"><i class="fa-solid fa-arrow-left"></i> Previous
      </button>
    </div>
    ...
    <div class="col-4 d-flex justify-content-end">
      <button :disabled="!loaded" type="button" class="btn btn-primary date-change-button"
              @click="handleDateInput(true)">Next <i class="fa-solid fa-arrow-right"></i></button>
    </div>
  </div>
  <div class="d-flex box-filter-separator">
    <hr class="col-12 separator-filter">
  ...
  <div class="sticky-table">
    <table v-if="loaded" class="table table-striped table-bordered planning-table">
      ...
      <template v-for="team in teams" :key="team.id">
        <tr>
          <td class="vertical-text w-5 text-center align-middle"
              :rowspan="team.members.length + team.technologies.length + 1">
            {{ team.name }}
          </td>
        </tr>
        <tr v-for="technology in team.technologies" class="evidence-bg-1">
          ...
          <td title="Planned monthly by TL / Planned monthly by PM"
              class="left_sticky l-200 w-8 align-middle text-center cell-p heading-tech-total">
          ...
          <td :class="setColorHour(tValue(['technologies', 'planned_tl', technology.id, project.id]), tValue(['technologies', 'planned_pm', technology.id, project.id]))"
              title="Planned monthly by TL / Planned monthly by PM" class="w-8 align-middle text-center cell-p"
        ...
        <tr v-for="member in team.members">
          <td class="w-20 align-middle cell-p left_sticky">{{ member.name }} <span title="Performance"
                                                                                   class="float-right">{{
              member.performance
            }}%</span></td>
  ...
  <div v-if="loaded">
    ...
    <div class="row">
      <div class="col-4 d-flex justify-content-start">
        <button type="button" class="btn btn-primary date-change-button" @click="handleDateInput(false)"><i
            class="fa-solid fa-arrow-left"></i> Previous
        </button>
      </div>
      <div class="col-4 d-flex justify-content-center align-items-center">
        <div class="week-inscription">{{ month_name }} {{ this.filter.year }} - {{ hours_count }} hours</div>
      ...
      <div class="col-4 d-flex justify-content-end">
        <button type="button" class="btn btn-primary date-change-button" @click="handleDateInput(true)">Next <i
...
export default {
  ...
  methods: {
    ...
    async getPlannings() {
      await axios.get('tl-planning/monthly', {
        params: {
          team_ids: this.filter.team_ids.map(obj => obj.id),
          project_ids: this.filter.project_ids.map(obj => obj.id),
          project_states: this.filter.project_states.map(obj => obj.id),
